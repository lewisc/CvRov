...
/* this program either takes in a single argument that
represents a fileof a picture, and parses that picture into it's
HSV respectively and then displays it.

If no arguments ar given it will try to read from the camera.

Note: No error checkig currently exists, so make it right.
*/

/*an HSV struct, useful for diagnostics primarily*/
typedef struct _HSV
{
    int hvalue;
    int svalue;
...
functions scope after it dereferences
*/
    ...
    int flags, void * param)
{
    if(event == CV_EVENT_LBUTTONDOWN)
    {
        ...
        IplImage const * const getcoords = 
        (*(IplImage const * const * const) (param));

        /*something to holdthe hsv image*/
        IplImage * hsv = cvCreateImage(cvGetSize(getcoords), 8, 3);
        cvCvtColor(getcoords, hsv, CV_BGR2HSV);

        /*get the HSV of the coords on the image*/
        HSV * targetval = (HSV*) malloc(sizeof(HSV));

        GetHsvFromXY(hsv,targetval, x, y);
...
int main( int argc, char** argv)
{
    ...
    cvSetMouseCallback(window, getHsvValue, &frame);
